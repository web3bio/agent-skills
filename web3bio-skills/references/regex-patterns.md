...
Machine-readable patterns for classifying an **extracted identity** string. For routing, run `.test(identity)` on each entry in **routing match order** (below); the **first match** yields the URL / display `platform`.  
Keep this file aligned with [platform-routing.md](platform-routing.md).

## `REGEX` (literal sources)

Same shapes as the former `utils/regex.ts`; copy into code if needed.
